# CodeIsland Runtime 展示端合同

[English](runtime-display-contract.md) | [文档索引](README_CN.md)

本文档定义 CodeIsland Runtime 和所有展示端之间的职责边界。Runtime 是 CodeIsland 的中心化控制面：它接入已配置 CLI source 的 hook 活动，归一化 session 和 pending action 状态，并把同一份状态暴露给一个或多个展示端。展示端可以是当前 WPF HUD、Web UI、移动端、硬件屏幕或第三方集成。

当前 Windows HUD 启动或连接 `CodeIsland.RuntimeHost`，并通过公开 REST/WebSocket contract 消费 Runtime 状态。Runtime 源码已拆到独立的 `CodeIsland-Runtime` 仓库；Windows 仓库保留为官方 WPF 展示客户端。
...
- `samples/external-display-console/`：只使用公开 REST/WebSocket contract 的 .NET 控制台示例。
...
`CodeIsland.Contracts` 中的 DTO 是公开展示端合同：
