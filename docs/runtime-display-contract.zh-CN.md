...
当前 Windows HUD 启动或连接 `CodeOrbit.RuntimeHost`，并通过公开 REST/WebSocket contract 消费 Runtime 状态。Runtime 源码已拆到独立的 `CodeOrbit-Runtime` 仓库；Windows 仓库保留为官方 WPF 展示客户端。
