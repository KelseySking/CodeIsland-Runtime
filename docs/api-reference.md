...
本文档面向 Windows HUD、Web、移动端、硬件屏幕和第三方展示端开发者，描述 Runtime 暴露的完整 REST/WebSocket 接口。接口源代码位于 `src/CodeIsland.Contracts/ApiDtos.cs` 和 `src/CodeIsland.Hub/CodeIslandApiHost.cs`。
...
| 参数 | 说明 |
| --- | --- |
| `--settings-dir <path>` | 指定 settings.json 所在目录。未指定时使用 `%APPDATA%\CodeIsland`。 |
...
如果你正在开发 Windows/Mac/Linux 桌面应用、Web UI、浏览器插件、IDE 插件、手机 companion、硬件屏幕或企业内部面板，请先阅读 `docs/integration-guide.md`。该文档说明每类应用应该如何启动或连接 Runtime、是否拥有 Runtime 生命周期、如何处理远程共享模式，以及如何做 Runtime 自动更新。
...
| `POST` | `/api/sources/{source}/install` | 安装或更新某个 source 的 CodeIsland hook。 |
| `POST` | `/api/sources/{source}/uninstall` | 卸载 CodeIsland 自己拥有的 hook entry。 |
