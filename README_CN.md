...
CodeOrbit Runtime 是 CodeOrbit 的中心化基座。它负责接入本机 CLI hook 事件，归一化会话、审批和问答状态，并通过带 token 认证的 REST/WebSocket 接口把同一份 Runtime 状态提供给多个展示端。
...
Windows HUD 只是官方展示客户端。它应该通过 `CodeOrbit.Contracts` 和 RuntimeHost/Bridge 可执行产物集成 Runtime，不应该继续编译依赖 Runtime 内部实现。
...
CLI hook -> CodeOrbit.Bridge -> Runtime named pipe -> Runtime 状态
```
...
CodeOrbit Runtime 支持通过**插件系统**扩展 CLI 源。这使你可以添加新的 AI CLI 工具支持，无需重新编译 Runtime。
...
生成包含 `CodeOrbit.RuntimeHost.exe`、`CodeOrbit.Bridge.exe` 和 `runtime-manifest.json` 的 Runtime ZIP：
