...
CodeOrbit Runtime is the centralized base process for CodeOrbit display clients. It ingests CLI hook events, normalizes sessions and pending approvals/questions, and exposes one token-authenticated REST/WebSocket contract to multiple displays.
...
- `CodeOrbit.Hub`: Runtime state, hook server, source service, REST API, WebSocket fan-out, and local token store.
- `CodeOrbit.RuntimeHost`: standalone Runtime process.
- `CodeOrbit.Bridge`: short-lived CLI hook bridge.
- Runtime-facing tests, docs, and external display samples.

The Windows HUD is a display client. It should consume this Runtime through `CodeOrbit.Contracts` plus RuntimeHost/Bridge executable artifacts, not by compiling against Runtime internals.
...
CLI hook -> CodeOrbit.Bridge -> Runtime named pipe -> Runtime state
```
...
```powershell
dotnet build CodeOrbit.Runtime.slnx
dotnet test CodeOrbit.Runtime.slnx
```
...
CodeOrbit Runtime supports custom CLI sources through a **plugin system**. This allows you to add support for new AI CLI tools without recompiling the Runtime.
...
Create a Runtime ZIP with `CodeOrbit.RuntimeHost.exe`, `CodeOrbit.Bridge.exe`, and `runtime-manifest.json`:
