...
public sealed class CodexPermissionRulesTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"CodeOrbit-rules-test-{Guid.NewGuid():N}");
    ...
    public void TryAppendAllowRule_UsesCodexHomeAndCreatesRulesFile()
    {
        ...
        var rulesPath = Path.Combine(codexHome, "rules", "CodeOrbit.rules");
    ...
    public void TryAppendAllowRule_ExpandsTildeCodexHome()
    {
        ...
        Assert.True(File.Exists(Path.Combine(_tempDir, "custom-codex", "rules", "CodeOrbit.rules")));
    }
